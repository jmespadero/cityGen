...
def optimizePolyline(points, widths, new_points, corners=[]):
    # Iterating along the cList3 points
    for i in range(len(points)):
        ...
        d_par = d_tot
        houses = 0
        # The vertex is added to the list and the corners list
        new_points.append(a)
        corners.append(a)

        # widths are added to the partial distance
        ...
        # Now, d_par is the extra distance divided by the number of houses. It will be added to each house to fit it completely to the whole wall
        d_par = d_par / houses
        sum = 0
...
def createHouse(a, b, c, d, heigh, name, material, data, corners):
    ...
    uvs = [Vector(((x - a).xy.length, x.z)) * 0.33 for x in vertex_list]
...
def main():
    ...
    if args.get('createDefenseWall', False):
        ...
        for i in range(1, len(wallVertices)):
            ...
            myVertex = [(v1[0], v1[1], 0), (v2[0], v2[1], 0), vertices3D[externalPoints[i]],
                        vertices3D[externalPoints[i - 1]]]
